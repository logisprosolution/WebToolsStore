...
{
    public partial class BuyInfo : BasePage
    {
        #region Parameter
        ...
        protected override void DoPrepareData()
        {
            ...
            loadEx.LoadPaymentType(ref ddl_type_buy, Enumerator.ConditionLoadEx.All);
        ...
        private void BindControl()//โหลดข้อมูล
        {
            if (base.dataId != -1)
            {
                ...
                if (ConvertHelper.IsDataExists(dt))
                {
                    ...
                    ddl_type_buy.SelectedValue = ConvertHelper.InitialValueDB(row, "payment_id");
                    txtDescription.Text = ConvertHelper.InitialValueDB(row, "header_remark");
        ...
        private void SaveInfo()//บันทึก
        {
            try
            {
                ...
                model.Doc_Header.header_customer_id = ConvertHelper.ToInt(hdf_header_name.Value);
                ...
                model.Doc_Header.payment_id = ConvertHelper.ToInt(ddl_type_buy.SelectedValue);
                model.Doc_Header.is_del = false;
                model.Doc_Header.is_enabled = true;
                model.Doc_Header.header_remark = txtDescription.Text;
                ...
                else
                {
                    ...
                    if (base.IsNewMode)
                    {
                        base.ShowMessageAndRedirect(SuccessMessage, typeof(BuyList));
                    }
                    else
                    {
                        base.ShowMessage(SuccessMessage);
        ...
        {
            base.RedirectToBackPage(typeof(BuyList));
        }
